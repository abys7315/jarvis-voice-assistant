...
fridayComs.push('search for youtube "your keywords" - to search on youtube ');
...
recognition.onresult = function (event) {
  ...
  if(localStorage.getItem("lang") === "en-US"){
    ...
    if(transcript.includes("switch to hindi")){
      ...
      recognition.stop()
      location.reload()
      readOutHindi("i am ready abhay")
    }
    if(transcript.includes("चेंज टू इंग्लिश।")){
    ...
    if (transcript.includes("tell about yourself")) {
      readOut(
        "sir, i am a jarvis, now a mini version of voice asisstant made for browsers using javascript by abhay sharma . I can do anything which can be done from a browser.I will get updates acording to market research"
      );
    }

    if (transcript.includes("tell them about the developer")) {
      readOut(
        "dear interviewers,he is abhay sharma. he is pursuing his bachelor of technology in vellore institute of technology , andhra pradesh.he has hands on experience with programming language like java,python and c++.he has a good understanding of data structures and algorithms.he is also comfortable working with web technologies such as h t m l,C S S,javascript .additionally, he has worked with s q l fro database management and has explored a w s services for cloud based solutions. he has started learning the basics of machine learning."
      );
    }
  ...
  // spotify
    ...
    if(transcript.includes("search in youtube")){
        ...
        let input=transcript.split("");
        input.splice(0,18);
        readOut(`here's the result of ${input.join("")}`);
        input.pop();
    ...
    if (transcript.includes("close all tabs")) {
      readOut("closing all tabs abhay")
      windowsB.forEach((e) => {
        e.close()
      })
    ...
    if (transcript.includes("top headlines")) {
      readOut("These are today's top headlines abhay")
      getNews()
